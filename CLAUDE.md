...
Код виходу 0 — усе зелено, 1 — є падіння або помилка на сторінці.

**Це не порушує «ніякого npm/збірки».** Гру ніяк не міняє і нічого не встановлює: тести
виконуються всередині справжньої `main.html` у headless Chromium через той самий
Playwright, що вже стоїть у `.venv-video` для рендер-скриптів (`test/_shot.py`).
Логіка гри — глобалі в `main.html`, імпортувати її в Node нема як, тому браузер тут
не забаганка, а єдиний спосіб дістатись коду.

- `test/run_tests.py` — раннер (Playwright, вантажить сторінку, друкує звіт)
- `test/tests.js` — самі перевірки, виконуються в контексті сторінки

**Що покрито (106 перевірок):** інваріанти дерев еволюції — реєстр (кожен монстр має
дерево і `_apply<Kind>Evo`, немає дерев-сиріт), структура вузлів, монотонність порогів,
узгодженість `computeEvoLevel`, і головне — **що ефекти реально застосовуються**.

Останнє ловить «фантомне дерево»: дерево є в `MON_DEV_TREES` і малюється в UI, але
`_apply<Kind>Evo` нічого не робить. У грі це виглядає як витрачені душі без ефекту і
**мовчить** — жодної помилки в консолі. Тест підміняє `getMonDev`/`getMonDevGolden`,
проганяє кожен вид на максимальному рівні по обох гілках і вимагає, щоб хоч одне поле
юніта змінилось.

**Пастка при написанні нових тестів:** `_apply<Kind>Evo` відповідає не лише за бонуси,
а й за **базову ініціалізацію** — слайм виставляє `ability`/`color`, водяний тримає щит
20% maxHp навіть без вибраної гілки. Тому інваріант нульового рівня перевіряє не
«нічого не змінилось», а «не виріс жоден бойовий стат і не ввімкнувся жоден `_evo*`».
...
- ВСІ 68 подій показуються РАЗ на кампанію (2026-07: раніше лише story:true; тактичні/гумор повторювались між ранами). Вибір кожної; вибір у кампанійній пам'яті `DLSiege_story` (`_storyShown(id)`/`_storyChoice(id)`), чиститься лише «Скинути все». `requires` перевіряється у пам'яті рану, потім кампанійній — ланцюги крізь рівні (act1_census→act4_soldier).
