...
function drawSkeletonMonster(unit, camY) {
    ...
    // Підлога масштабу 0.75: телефонні скелети дрібні (size 25 → sc 0.375), і
    // фізично чесний короткий крок давав ~4.4 кроки/с — несамовите мигтіння.
    // З підлогою виходить ~3 кроки/с; невелике проковзування непомітне,
    // шалені ноги — помітні. Кап 8.5 t-од/с — стеля темпу на спринті.
    if (_skDx > 0.05) {
        const _adv = _skDx * (Math.PI / 2.2) / (25 * Math.max(_skSc, 0.75));
        unit._skT += Math.min(_adv, _skDt * 8.5);
    } else {
